...
export default function MobileStyles() {
  return (
    ...
    /* Make sure the app surface is dark everywhere */
    html, body, #root, .app {
    ...
    /* iOS/Safari form-control resets to avoid unwanted white backgrounds */
    ...
    /* === Main/Home controls (mobile fixes) ===
       These styles target the homepage input/select/buttons so they don't
       inherit iOS default white backgrounds on phones. We keep them scoped
       and slightly stronger on narrow/mobile only. */
    ...
    /* Prompt input (fix white bg on iOS) */
    .prompt-input {
      width: 100%;
      height: 44px;
      border-radius: 12px;
    ...
    .prompt-input:disabled { opacity: .6; }
    ...
    .suggest-btn:hover { background: rgba(255,255,255,0.08); }
    .suggest-btn:active { transform: scale(.98); }
    .suggest-btn:disabled { opacity: .5; cursor: not-allowed; }

    /* Style dropdown (fix white bg + native arrow) */
    .style-select {
      width: 100%;
      height: 44px;
      border-radius: 12px;
      ...
      transition: box-shadow .2s ease, border-color .2s ease, background .2s ease;
      background-image:
        linear-gradient(45deg, transparent 50%, #f3f5f9 50%),
        linear-gradient(135deg, #f3f5f9 50%, transparent 50%),
    ...
    .style-select:disabled { opacity: .6; }
    ...
    .generate-btn:hover { background: #e55a2b; box-shadow: 0 10px 24px rgba(255,107,53,.45); }
    .generate-btn:active { transform: translateY(1px); }
    .generate-btn:disabled { opacity: .6; box-shadow: none; cursor: not-allowed; }
    ...
    /* Draw (secondary) — was white on iOS, force dark chip */
    .draw-btn {
      height: 44px;
      border-radius: 12px;
      border: var(--border-soft-dark);
    ...
    .draw-btn:hover { background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.12); }
    ...
    /* Compact layout on narrow phones; tablets keep existing look */
